...
func (g *LibraryGenerator) GenerateLibrary() error {
	...
	for _, methodsGroup := range groups {
		...
		for _, method := range groupMethods {
			...
			// Non-context wrapper delegates to context version.
			builder.WriteString(fmt.Sprintf("func (c *%s) %s(%s) (map[string]interface{}, error) {\n", className, funcName, strings.Join(params, ", ")))
			builder.WriteString(fmt.Sprintf("\treturn c.%sContext(context.Background(), %s)\n", funcName, strings.Join(paramNames, ", ")))
			builder.WriteString("}\n\n")

			// Context version with the actual implementation.
			contextParams := append([]string{"ctx context.Context"}, params...)
			builder.WriteString(fmt.Sprintf("func (c *%s) %sContext(ctx context.Context, %s) (map[string]interface{}, error) {\n", className, funcName, strings.Join(contextParams, ", ")))
			if requiresAuth {
				builder.WriteString(fmt.Sprintf("\tresp, err := c.AuthenticatedRequestContext(ctx, %s)\n", buildArgsString(args)))
			} else {
				builder.WriteString(fmt.Sprintf("\tresp, err := c.MsfRequestContext(ctx, %s)\n", buildArgsString(args)))
			}
